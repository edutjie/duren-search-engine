class IdMap:
    ...
    def __contains__(self, key):
        return (
            key in self.str_to_id.keys()
            if isinstance(key, str)
            else key in self.id_to_str
        )
...
def merge_and_sort_posts_and_tfs(posts_tfs1, posts_tfs2):
    ...
    while i < len(posts_tfs1) and j < len(posts_tfs2):
        ...
        if doc_id1 == doc_id2:
            res.append((doc_id1, tf1 + tf2))
            i += 1
            j += 1
        elif doc_id1 < doc_id2:
            res.append((doc_id1, tf1))
            i += 1
        else:
            res.append((doc_id2, tf2))
            j += 1

    # append remaining elements
    while i < len(posts_tfs1):
        res.append(posts_tfs1[i])
        i += 1
    while j < len(posts_tfs2):
        res.append(posts_tfs2[j])
        j += 1
...
if __name__ == "__main__":
    ...
    assert merge_and_sort_posts_and_tfs(
        [(1, 34), (3, 2), (4, 23)], [(1, 11), (2, 4), (4, 3), (6, 13)]
    ) == [
        (1, 45),
        (2, 4),
        (3, 2),
        (4, 26),
        (6, 13),
    ], "merge_and_sort_posts_and_tfs salah"
